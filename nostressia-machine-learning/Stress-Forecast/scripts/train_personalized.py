...
def _resolve_personalized_notebook_path() -> Path:
    env_path = os.environ.get("PERSONALIZED_FORECAST_NOTEBOOK_PATH")
    if env_path:
    ...
    preferred = (
        ...
        / "Stress-Forecast"
        / "notebooks"
        / "experiments_personalized"
        / "11_bagging_tree_personalized_forecast.ipynb"
    )
    if preferred.exists():
        return preferred

    return REPO_ROOT / "nostressia-machine-learning" / "Stress-Forecast" / "notebooks" / "personalized_forecast.ipynb"


NOTEBOOK_PATH = _resolve_personalized_notebook_path()
DATASET_PATH = (
...
def _is_mlflow_storage_fresh(repo_root: Path) -> bool:
    """Return True when local MLflow storage looks reset/empty."""
    mlruns_dir = repo_root / "mlruns"
    if not mlruns_dir.exists():
        return True
    # Experiment folders are numeric IDs (e.g., 0, 12345)
    experiment_dirs = [p for p in mlruns_dir.iterdir() if p.is_dir() and p.name.isdigit()]
    return len(experiment_dirs) == 0
...
def train_personalized(
    ...
    # If MLflow storage has been reset (`rm -rf mlruns`) but state file still
    # remembers previous personalized training, gate logic can skip retraining.
    # Reset personalized state automatically so first run after reset can log again.
    if _is_mlflow_storage_fresh(REPO_ROOT):
        existing_users_state = state.personalized.get("users", {}) if isinstance(state.personalized, dict) else {}
        if existing_users_state:
            print("Detected fresh MLflow storage; clearing personalized state gates for re-bootstrap training.")
