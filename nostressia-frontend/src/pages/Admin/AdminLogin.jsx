      ...
      <div className="max-w-md w-full bg-surface-elevated glass-panel dark:bg-surface rounded-2xl shadow-xl overflow-hidden border border-border-subtle dark:border-border glass-panel-strong">
        ...
        <div className="bg-gradient-to-br from-brand-warning/20 via-brand-warning/10 to-brand-primary/20 dark:from-brand-warning/20 dark:via-surface/80 dark:to-brand-primary/20 p-8 text-center relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col items-center">
                ...
                />
                <h2 className="text-2xl font-bold text-text-primary dark:text-text-primary">Admin Portal</h2>
                <p className="text-text-secondary dark:text-text-muted text-sm mt-1 font-medium">Nostressia Management System</p>
            </div>
        </div>
        ...
        <div className="p-8">
          <form onSubmit={handleLogin} onKeyDown={handleFormKeyDown} className="space-y-6">
            ...
            <div className="space-y-4">
                {/* Username Input */}
                <div>
                    <label className="block text-xs font-bold text-text-muted dark:text-text-muted uppercase mb-1 ml-1">Username</label>
                    <div className="relative">
                        <User className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted dark:text-text-muted w-5 h-5" />
                        <input
                            type="text"
                ...
                {/* Password Input */}
                <div>
                    <label className="block text-xs font-bold text-text-muted dark:text-text-muted uppercase mb-1 ml-1">Password</label>
                    <div className="relative">
                        <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted dark:text-text-muted w-5 h-5" />
                        <input
                            type="password"
                            placeholder="••••••••"
                            className="w-full pl-12 pr-4 py-3 glass-input border border-border rounded-xl focus:bg-surface-elevated glass-panel focus:ring-2 focus:ring-brand-primary/20 focus:outline-none transition-all text-text-primary"
          ...
          <div className="mt-8 pt-6 border-t border-border-subtle text-center">
            <Link to="/" className="inline-flex items-center gap-2 text-sm text-text-muted dark:text-text-muted hover:text-text-secondary dark:hover:text-text-primary transition-colors">
