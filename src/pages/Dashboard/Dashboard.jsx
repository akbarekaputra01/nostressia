...
export default function Dashboard() {
  ...
  function handleSaveForm(e) {
    ...
    if (sleepHours === "" || sleepHours < 0 || sleepHours > 24)
      return alert("Isi jam tidur valid (0-24).");
    if (studyHours === "" || studyHours < 0 || studyHours > 24)
      return alert("Isi jam belajar valid (0-24).");
    if (socialHours === "" || socialHours < 0 || socialHours > 24)
      return alert("Isi jam sosial valid (0-24).");

    const successTitleText = hasSubmittedToday
      ? "Data Diperbarui!"
      : "Berhasil!";
    const successDescText = hasSubmittedToday
      ? "Perubahan data hari ini telah disimpan."
      : "Data telah disimpan.";
    setSuccessModal({
      visible: true,
      title: successTitleText,
      text: successDescText,
    });
    ...
    const newColor =
      finalScore > 60 ? brandRed : finalScore > 30 ? brandOrange : brandGreen;
      ...
      <main className="max-w-[1400px] mx-auto p-6 md:p-8 lg:p-10 pt-28">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div style={{ perspective: 1500 }} className="w-full h-full">
              ...
              >
                ...
                >
                  ...
                  <div className="text-center my-4 flex-grow relative z-10">
                    ...
                    {hasSubmittedToday ? (
                      <p className="text-green-600 font-semibold">
                        Berdasarkan data inputanmu
                      </p>
                    ) : (
                      <p className="text-gray-400 font-semibold">
                        Silakan isi data untuk melihat prediksi
                      </p>
                    )}
                  ...
                  <button
                    className="w-full py-3 rounded-xl font-bold text-white shadow-md pressable transition-colors duration-300 relative z-10"
                  ...
                  >
                    {hasSubmittedToday ? (
                      <>
                        <i className="ph ph-pencil-simple mr-2" /> Edit Data
                        Prediksi
                      </>
                    ) : (
                      <>
                        <i className="ph ph-note-pencil mr-2" /> Isi Data
                        Prediksi Stres
                      </>
                    )}
                  </button>
                ...
                >
                  ...
                  <form
                    ...
                    className="flex-grow overflow-y-auto pr-2 flex flex-col gap-4 transition-all duration-500 custom-scroll"
                    style={{
                  ...
                  >
                    ...
                    <div>
                      ...
                      {!isEditingGpa ? (
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            type="button"
                            className="w-9 h-9 rounded-xl flex items-center justify-center bg-blue-100 text-blue-600 hover:bg-blue-200 transition-colors"
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            type="button"
                            className="w-9 h-9 rounded-xl flex items-center justify-center bg-green-100 text-green-600 hover:bg-green-200 transition-colors"
                            onClick={() =>
                              handleGpaSave(
                          ...
                          <button
                            type="button"
                            className="w-9 h-9 rounded-xl flex items-center justify-center bg-red-100 text-red-600 hover:bg-red-200 transition-colors"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        step="0.5"
                        placeholder="Misal: 7"
                        className="w-full p-3 border border-white/50 bg-white/50 rounded-lg focus:ring-2 focus:ring-blue-400 placeholder-gray-400"
                      />
                    </div>
                    <div>
                      <label className="text-sm font-semibold text-gray-800 mb-2 block">
                      ...
                      <div className="flex justify-around">
                        {moods.map((emo, idx) => (
                          <button
                            key={idx}
                            type="button"
                            className={`w-12 h-12 rounded-xl flex items-center justify-center text-3xl transition-transform ${
                              moodIndex === idx
                                ? "scale-110 shadow-lg"
                                : "hover:scale-105"
                            }`}
                    ...
                    >
                      {hasSubmittedToday ? (
                        ...
                        <span className="flex items-center justify-center">
                          <i className="ph ph-check-circle mr-2" /> Simpan Data
                        </span>
                      )}
                    </button>
                  </form>
          ...
          >
            <header className="flex justify-between items-center mb-4">
              <button
                className="icon-btn text-gray-600 hover:text-gray-900 transition-colors"
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-semibold text-gray-600 italic">
                        "
                        {dayDetail.level > 60
                          ? "Istirahatlah, kamu butuh jeda."
                          : "Hari yang cukup seimbang!"}
                        "
                      </p>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <div>
                      <div className="flex justify-between text-xs font-bold text-gray-600 mb-1">
                        ...
                        <span>{dayDetail.sleep} jam</span>
                      </div>
                      <div className="h-2 w-full bg-gray-200 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-purple-500 rounded-full"
                          style={{ width: `${(dayDetail.sleep / 10) * 100}%` }}
                        />
                      </div>
                    </div>
                    <div>
                      <div className="flex justify-between text-xs font-bold text-gray-600 mb-1">
                        <span className="flex items-center gap-1">
                          <i className="ph ph-book-open text-blue-500" />{" "}
                          Belajar
                        </span>
                        <span>{dayDetail.study} jam</span>
                      </div>
                      <div className="h-2 w-full bg-gray-200 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-blue-500 rounded-full"
                          style={{ width: `${(dayDetail.study / 12) * 100}%` }}
                        />
                      </div>
                    </div>
                    <div>
                      <div className="flex justify-between text-xs font-bold text-gray-600 mb-1">
                        <span className="flex items-center gap-1">
                          <i className="ph ph-users text-orange-500" /> Sosial
                        </span>
                        <span>{dayDetail.social} jam</span>
                      </div>
                      <div className="h-2 w-full bg-gray-200 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-orange-500 rounded-full"
                          style={{ width: `${(dayDetail.social / 8) * 100}%` }}
                        />
                      </div>
                    </div>
                  </div>
        ...
        <div className="mt-8 grid grid-cols-1">
          <section className="col-span-4 flex flex-col md:flex-row items-center justify-between p-5 md:p-8 rounded-[20px] bg-white/40 backdrop-blur-md border border-white/20 shadow-xl gap-4">
            <div className="flex items-center gap-3">
              <i
                className="ph ph-lightbulb-filament text-3xl"
                style={{ color: brandOrange }}
              />
            ...
            <button
              ...
              onClick={() => {
                let n;
                do {
                  n = quotes[Math.floor(Math.random() * quotes.length)];
                } while (n === quote);
                setQuote(n);
              }}
              style={{ border: `2px solid ${brandOrange}`, color: brandOrange }}
            >
              <i className="ph ph-arrows-clockwise" /> Kutipan Baru
            </button>
          </section>
          <div className="mt-6 flex flex-wrap justify-center gap-6">
            <div
              className="flex-1 min-w-[280px] max-w-[360px] p-6 rounded-[20px] bg-white/40 backdrop-blur-md border border-white/20 shadow-xl cursor-pointer hover:shadow-2xl hover:-translate-y-1 transition"
              onClick={() =>
                openResource(
                  "Tips Tidur Nyenyak",
                  "Cahaya biru dari HP dapat mengganggu produksi melatonin. Matikan perangkat 1 jam sebelum tidur.",
                  "ph ph-moon-stars"
                )
              }
            >
              <i className="ph ph-moon-stars text-4xl text-purple-600 mb-3" />
              <h3 className="text-lg font-bold">
                Drift off with sleep stories
              </h3>
              <p className="text-sm text-gray-600 mt-1">
                Cerita dan musik untuk tidur lebih nyenyak.
              </p>
            </div>
            <div
              className="flex-1 min-w-[280px] max-w-[360px] p-6 rounded-[20px] bg-white/40 backdrop-blur-md border border-white/20 shadow-xl cursor-pointer hover:shadow-2xl hover:-translate-y-1 transition"
              onClick={() =>
                openResource(
                  "Meditasi 1 Menit",
                  "Tarik napas 4 detik, tahan 4 detik, hembus 4 detik. Ulangi 5 kali.",
                  "ph ph-wind"
                )
              }
            >
              <i className="ph ph-timer text-4xl text-blue-600 mb-3" />
              <h3 className="text-lg font-bold">Learn to meditate</h3>
              <p className="text-sm text-gray-600 mt-1">
                Teknik pernapasan mudah untuk mulai meditasi.
              </p>
            </div>
            <div
              className="flex-1 min-w-[280px] max-w-[360px] p-6 rounded-[20px] bg-white/40 backdrop-blur-md border border-white/20 shadow-xl cursor-pointer hover:shadow-2xl hover:-translate-y-1 transition"
              onClick={() =>
                openResource(
                  "Membangun Kebiasaan",
                  "Mulai dari yang kecil. Rule of 2 Minutes: jika butuh <2 menit, lakukan sekarang.",
                  "ph ph-target"
                )
              }
            >
              <i className="ph ph-target text-4xl text-yellow-600 mb-3" />
              <h3 className="text-lg font-bold">Build healthy habits</h3>
              <p className="text-sm text-gray-600 mt-1">
