...
import { BASE_URL } from "../../api/config"; // <--- 1. KITA IMPORT INI DARI CONFIG KAMU
...
const quotesList = [
  ...
  {
    text: "Resting doesn't mean stopping, it means recharging.",
    author: "Mindfulness",
  },
  {
    text: "You don't have to be productive all the time. Just breathe.",
    author: "Self Care",
  },
  {
  ...
  {
    text: "Peace begins when you stop expecting others to understand you.",
    author: "Inner Peace",
  },
  {
    text: "Flowers don't bloom simultaneously. Be patient with your process.",
    author: "Nature's Law",
  },
];
...
const resourcesList = [
  {
    ...
    detail:
      "Blue light tricks your brain into thinking it's still daytime. Use 'Night Shift' mode or, even better, read a physical book before sleeping.",
    icon: "ph-moon-stars",
  ...
  },
  {
    ...
    detail:
      "Inhale through nose (4 sec), Hold breath (7 sec), Exhale through mouth making a 'whoosh' sound (8 sec). Repeat 4 cycles.",
    icon: "ph-wind",
  ...
  },
  {
    ...
    detail:
      "If a task takes less than 2 minutes, do it NOW. Don't delay or write it down, just do it immediately.",
...
// --- NEW HELPER: Translate ML Result to UI Score ---
...
function PopupModal({ visible, onClose, title, text, iconClass, success }) {
  useEffect(() => {
    if (!visible) return;
    let t;
    if (success) t = setTimeout(onClose, 1000);
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div
        className="relative w-11/12 max-w-sm p-6 rounded-xl bg-white/75 border border-white/20 shadow-2xl"
        style={{ backdropFilter: "blur(8px)" }}
      >
        {success && (
          <div
            className="absolute inset-0 rounded-xl"
            style={{ boxShadow: `0 0 0 4px ${brandOrange}`, opacity: 0.08 }}
          />
        )}
        <button
        ...
        <div className="flex flex-col items-center gap-3 text-center">
          <div
            className="text-4xl"
            style={{ color: success ? brandGreen : brandBlue }}
          >
...
export default function Dashboard() {
  ...
  // --- UPDATED: HANDLE SAVE WITH BACKEND CONNECTION (DYNAMIC URL) ---
  async function handleSaveForm(e) {
    ...
    if (studyHours === "" || studyHours < 0 || studyHours > 24)
      return alert("Please enter valid study hours (0-24).");
    if (extraHours === "" || extraHours < 0 || extraHours > 24)
      return alert("Please enter valid extra hours (0-24).");
    if (socialHours === "" || socialHours < 0 || socialHours > 24)
      return alert("Please enter valid social hours (0-24).");
    if (physicalHours === "" || physicalHours < 0 || physicalHours > 24)
      return alert("Please enter valid physical activity hours (0-24).");
    ...
    try {
      ...
      // 3. Kirim Request ke API (PAKAI BASE_URL DARI CONFIG)
      // Dulu: "http://127.0.0.1:8000/api/predict/current-stress"
      // Sekarang jadi dinamis sesuai config.js
      ...
      if (!response.ok) {
        throw new Error(
          apiData.detail || "Terjadi kesalahan saat menghubungi server."
        );
      }
      ...
      const successTitleText = hasSubmittedToday
        ? "Data Updated!"
        : "Analysis Complete!";
      setSuccessModal({
        visible: true,
      ...
      if (BASE_URL.includes("localhost") || BASE_URL.includes("127.0.0.1")) {
        pesanError +=
          "👉 Cek terminal Backend, pastikan 'uvicorn' sudah jalan.";
      } else {
  ...
  function handleGpaSave(val) {
    ...
    if (Number.isNaN(num) || num < 0 || num > 4)
      return alert("GPA must be between 0 - 4");
    setGpa(num);
    ...
    >
      ...
      <main className="max-w-[1400px] mx-auto p-6 md:p-8 lg:p-10 pt-28">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              >
                ...
                >
                  ...
                  <div className="text-center my-4 flex-grow relative z-10">
                    ...
                    <h1
                      className="text-6xl md:text-7xl font-extrabold my-4"
                      style={{
                        color: hasSubmittedToday ? brandBlue : "#9ca3af",
                      }}
                    >
                      {stressScore}%
                    </h1>
                    {hasSubmittedToday ? (
                      ...
                      <p className="text-gray-400 font-semibold">
                        Please fill in data to see prediction
                      </p>
                    )}
                  ...
                  <h4 className="text-base font-bold text-gray-800 mb-3 relative z-10">
                    Last 7 Days Trend
                  </h4>
                        ...
                        >
                          <div className="relative h-20 w-full flex justify-center">
                            <div
                              ...
                              style={{ background: dotColor, bottom }}
                            />
                          </div>
                ...
                >
                  ...
                  <form
                    ...
                    style={{
                      ...
                      transform: successModal.visible
                        ? "scale(0.95)"
                        : "scale(1)",
                      pointerEvents: successModal.visible ? "none" : "auto",
                    }}
                  >
                    {/* GPA */}
                    <div>
                      ...
                      {!isEditingGpa ? (
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            type="button"
                            className="w-9 h-9 rounded-xl flex items-center justify-center bg-blue-100 text-blue-600 hover:bg-blue-200 transition-colors cursor-pointer"
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          step="0.5"
                          placeholder="0"
                          className="w-full p-2.5 border border-white/50 bg-white/50 rounded-lg focus:ring-2 focus:ring-blue-400 placeholder-gray-400"
                        />
                      </div>
                      ...
                      <div className="flex justify-around">
                        {moods.map((emo, idx) => (
                          <button
                            key={idx}
                            type="button"
                            className={`w-10 h-10 md:w-12 md:h-12 rounded-xl flex items-center justify-center text-2xl md:text-3xl transition-transform cursor-pointer ${
                              moodIndex === idx
                                ? "scale-110 shadow-lg"
                                : "hover:scale-105"
                            }`}
                  ...
                  {successModal.visible && (
                    <div
                      className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-white/60 backdrop-blur-md rounded-[20px]"
          ...
          {/* CALENDAR */}
          <section
            className="col-span-1 md:col-span-2 p-6 md:p-8 rounded-[20px] bg-white/40 backdrop-blur-md border border-white/20 shadow-xl relative overflow-hidden"
            style={{ minHeight: 640 }}
          >
            <header className="flex justify-between items-center mb-4">
              <button
                className="icon-btn text-gray-600 hover:text-gray-900 transition-colors cursor-pointer"
              ...
              <button
                ...
                onClick={() => changeMonth(1)}
              >
                <i className="ph ph-arrow-right text-xl" />
              </button>
            </header>
            ...
            <div className="grid grid-cols-7 gap-2">
              ...
              {[...Array(daysInMonth)].map((_, i) => {
                ...
                return (
                  <div
                    key={day}
                    className={`flex items-center justify-center h-12 rounded-xl font-semibold text-sm cursor-pointer transition-all duration-200 ${
                      isSel ? "scale-105 shadow-md" : "hover:bg-white/50"
                    }`}
                    onClick={() => handleDateClick(day)}
                    style={{
                      background: isSel ? brandBlue : "transparent",
                      color: isSel ? "white" : "#333",
                      border: hasData ? `2px solid ${has.color}40` : "none",
                    }}
                  >
              ...
              >
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      <h4 className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">
                        Daily Recap
                      </h4>
                  ...
                  >
                    <div>
                      <div className="flex justify-between text-xs font-bold text-gray-600 mb-1">
                        <span className="flex items-center gap-1">
                          <i className="ph ph-moon-stars text-purple-500" />{" "}
                          Sleep
                        </span>
                        <span>{dayDetail.sleep} hrs</span>
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between text-xs font-bold text-gray-600 mb-1">
                        <span className="flex items-center gap-1">
                          <i className="ph ph-book-open text-blue-500" /> Study
                        </span>
                        <span>{dayDetail.study} hrs</span>
                      </div>
                      <div className="h-2 w-full bg-gray-200 rounded-full overflow-hidden">
                        <div
                    ...
                    <div>
                      <div className="flex justify-between text-xs font-bold text-gray-600 mb-1">
                        <span className="flex items-center gap-1">
                          <i className="ph ph-medal text-pink-500" /> Extra
                        </span>
                        <span>{dayDetail.extra || 0} hrs</span>
                      </div>
                      <div className="h-2 w-full bg-gray-200 rounded-full overflow-hidden">
                        <div
                    ...
                    <div>
                      <div className="flex justify-between text-xs font-bold text-gray-600 mb-1">
                        <span className="flex items-center gap-1">
                          <i className="ph ph-users text-orange-500" /> Social
                        </span>
                        <span>{dayDetail.social} hrs</span>
                      </div>
                      <div className="h-2 w-full bg-gray-200 rounded-full overflow-hidden">
                        <div
                    ...
                    <div>
                      <div className="flex justify-between text-xs font-bold text-gray-600 mb-1">
                        <span className="flex items-center gap-1">
                          <i className="ph ph-sneaker text-teal-500" /> Exercise
                        </span>
                        <span>{dayDetail.physical || 0} hrs</span>
                      </div>
                      <div className="h-2 w-full bg-gray-200 rounded-full overflow-hidden">
                        <div
            ...
            <div className="relative z-10 p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-6">
              ...
              >
                <span className="flex items-center gap-2">
                  <i
                    className={`ph ph-arrows-clockwise text-xl transition-transform duration-700 ${
                      isQuoteAnimating ? "rotate-180" : ""
                    }`}
                  ></i>
                  <span>New Quote</span>
                </span>
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
            {resourcesList.map((item) => (
              <div
                key={item.id}
                onClick={() =>
                  openResource(item.title, item.detail, `ph ${item.icon}`)
                }
              ...
              >
                ...
                <div
                  className={`absolute -right-10 -top-10 w-32 h-32 bg-gradient-to-br ${item.color} rounded-full opacity-0 group-hover:opacity-10 blur-2xl transition-opacity duration-500`}
                />

                <div className="relative z-10">
                  <div className="flex justify-between items-start mb-4">
                    <div
                      className={`w-14 h-14 rounded-2xl ${item.bg} flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform duration-300`}
                    >
                      <i className={`ph ${item.icon} text-3xl ${item.text}`} />
                    </div>
                    <span
                      className={`text-[10px] font-bold uppercase tracking-wider py-1 px-2 rounded-lg ${item.bg} ${item.text} opacity-70`}
                    >
                      {item.category}
                    </span>
                  </div>
                  ...
                  <div
                    className={`flex items-center text-sm font-bold ${item.text} opacity-0 -translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300`}
                  >
      ...
      {resourceModal.visible && (
        <PopupModal
          ...
          onClose={() =>
            setResourceModal({ visible: false, title: "", text: "", icon: "" })
          }
          title={resourceModal.title}
          text={resourceModal.text}
          iconClass={resourceModal.icon}
          success={false}
        />
      )}
    </div>
