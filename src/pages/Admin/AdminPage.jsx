...
import { 
  ...
  Plus, Trash2, X, User, FolderPlus, UserCircle, Tag 
} from "lucide-react";
...
export default function AdminPage() {
  ...
  // --- FUNGSI LOGOUT ---
  const handleLogout = () => {
    localStorage.removeItem("adminAuth"); // Hapus sesi
        ...
        <div className="flex items-center gap-6">
            <div className="hidden md:flex items-center gap-3 text-sm font-medium bg-gray-100 px-4 py-2 rounded-full border border-gray-200">
                ...
                <div className="flex flex-col leading-tight">
                    ...
                    <span className="text-[10px] text-gray-500 uppercase tracking-wider">ID: {currentUser.id}</span>
                </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all relative overflow-hidden group">
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
              ...
              <span className="text-3xl font-bold text-gray-800">{quotes.length}</span>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-2xl">
              <h3 className="text-xl font-bold text-gray-800 flex items-center gap-2"><Sparkles className="text-orange-500" /> Manage Motivation</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              <form onSubmit={handleAddQuote} className="bg-orange-50/50 p-5 rounded-xl border border-orange-100 space-y-4 relative">
                <div className="absolute top-3 right-3 flex items-center gap-1.5 text-[10px] text-orange-600/70 bg-white/60 px-2 py-1 rounded-full"><Tag size={12}/> {currentUser.name}</div>
                <h4 className="font-bold text-sm text-orange-800 uppercase tracking-wide">Tambah Quote Baru</h4>
                <div>
                    <label className="text-xs font-bold text-gray-500 ml-1 mb-1 block">KATA-KATA QUOTE</label>
                    <textarea placeholder="Contoh: Life is a journey..." className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-400 focus:outline-none bg-white" rows="2"
                    value={quoteForm.text} onChange={(e) => setQuoteForm({...quoteForm, text: e.target.value})} />
                </div>
                <div className="flex gap-3">
                    <div className="flex-1">
                        <label className="text-xs font-bold text-gray-500 ml-1 mb-1 block">AUTHOR</label>
                        <input type="text" placeholder="Contoh: Anonymous" className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-400 focus:outline-none bg-white"
                        value={quoteForm.author} onChange={(e) => setQuoteForm({...quoteForm, author: e.target.value})} />
                    </div>
                    <div className="flex items-end"><button type="submit" className="h-[46px] px-6 bg-orange-500 text-white font-bold rounded-xl hover:bg-orange-600 transition-colors flex items-center gap-2 shadow-md cursor-pointer"><Plus size={18} /> Tambah</button></div>
                </div>
              </form>
              <div className="space-y-3">
                {quotes.map((q) => (
                  <div key={q.id} className="flex justify-between items-start p-4 bg-white border border-gray-100 rounded-xl shadow-sm hover:border-orange-200 transition-colors">
                    <div>
                      <p className="text-gray-800 font-medium italic text-lg">"{q.text}"</p>
                      <div className="flex flex-wrap items-center gap-2 mt-2">
                        <div className="h-0.5 w-4 bg-orange-300"></div>
                        <span className="text-sm font-bold text-gray-500 uppercase tracking-wide">{q.author}</span>
                        <span className="text-xs text-gray-300 mx-1">•</span>
                        <span className="text-[10px] font-medium text-gray-400 bg-gray-50 px-2 py-0.5 rounded border border-gray-100">By: {q.uploaderName} ({q.uploaderId})</span>
                      </div>
                    </div>
                    <button onClick={() => handleDeleteQuote(q.id)} className="text-gray-300 hover:text-red-500 p-2 hover:bg-red-50 rounded-lg transition-all"><Trash2 size={18} /></button>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="w-full md:w-1/3 space-y-4">
                <div className="bg-white p-5 rounded-xl border border-blue-100 shadow-sm sticky top-0">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <span className="text-[9px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded border border-blue-100">{currentUser.id}</span>
                    </div>
